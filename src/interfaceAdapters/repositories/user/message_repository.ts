...
import {
  IMessageModel,
  MessageModel,
} from "../../../frameworks/database/models/message_model";
...
export class MessageRepository implements IMessageRepository {
  ...
  async getMessagesByChatId(chatId: string): Promise<IMessageModel[]> {
    return await MessageModel.find({ chatId }).sort({ createdAt: 1 }).populate("senderId","_id Name profileImage").populate("receiverId","_id Name profileImage").exec();
  }
}

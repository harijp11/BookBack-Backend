...
export class UpdateDealTypeUseCase implements IUpdateDealTypeUseCase{
    constructor(
        ...
        private dealTyepeRepository:IDealTypeRepository
    ){}

    async execute(_id: string, name: string, description: string): Promise<void> {
        const deal = await this.dealTyepeRepository.findByName(name)

        if(deal){
            throw new CustomError("Deal type Name already existed",400)
        }

        await this.dealTyepeRepository.findByIdAndUpdateDealType(_id,name,description) 
    }
}
...
import { IBookWithDistance } from "../../../entities/types/IBookWithDistance";

@injectable()
export class GetAllUserAvailbleBooksUseCase
  implements IGetAllUserAvailableBooksUseCase
{
  constructor(
  ...
  async execute({
    ...
    const locationBasedFilteredBooks =
      await this._bookRepository.findLocationBasedFilteredBooks(
        latitude,
        longitude,
        maxDistance,
        currentLimit,
    ...
    if (!locationBasedFilteredBooks) {
      throw new CustomError(
        BOOK_ERROR_RESPONSES.BOOKS_NOT_FOUND,
    ...
    const rawBooks = getBooks(); // contains `distance`
    const mappedBooks = Mapper.toBookEntityList(rawBooks);

    if (!mappedBooks || count === undefined) {
      throw new CustomError(
        BOOK_ERROR_RESPONSES.BOOKS_NOT_FOUND,
        HTTP_STATUS.NOT_FOUND
      );
    }
    const booksWithDistance: IBookWithDistance[] = mappedBooks.map(
      (book, idx) => ({
        ...book,
        distance: rawBooks[idx]?.distance ?? 0, 
      })
    );
